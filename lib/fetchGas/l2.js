...
const ARB_FALLBACK_L2_GWEI = 0.25;         // final fallback
const ARB_FALLBACK_L1_DATA_ETH = 0.000015; // final fallback
...
async function rollupGasPrices(chain, timeout) {
  for (const rpc of toRpcList(chain.rpc)) {
    try {
      ...
      const l1DataGas = l1Data && l1Wei
        ? Math.max(Math.round(l1Data / l1Wei), chain.l1DataGas || 30000)
        : chain.l1DataGas;

      const cand = buildCandidate(
        chain,
        rpcProviderLabel('rollup', rpc),
        l2Wei ? l2Wei / 1e9 : null,
        l1Wei ? l1Wei / 1e9 : null,
        l1DataGas
      );
...
async function rpcWithEthBase(chain, timeout) {
  ...
  for (const rpc of toRpcList(chain.rpc)) {
    try {
      ...
      const cand = buildCandidate(
        chain,
        rpcProviderLabel('rpc-l1-base', rpc),
        gwei,
        l1GasPrice,
        chain.l1DataGas
      );
...
async function fetchArbGas(chain) {
  ...
  for (const layer of layers) {
    try {
      ...
      if (!l2GasPrice) {
        const v = validL2GasPrice(res.gasPriceGwei);
        if (v) l2GasPrice = v;
      }

      if (!l1DataPrice) {
        const val = Number(res.l1DataPrice);
        if (Number.isFinite(val) && val > 0) l1DataPrice = val;
      }
  ...
  // safe fee
  const feeNative =
    (l2GasPrice / 1e9) * gasLimit + l1DataPrice;
  ...
  const candidate =
    arbBuildCandidate(provider, l2GasPrice, l1DataPrice) ||
  ...
  recordFetchError(cacheKey, 'no arb gas candidate');
...
async function fetchL2Gas(chain) {
  ...
  recordFetchError(cacheKey, lastError || 'no l2 gas candidate');
  const cached = getFreshCache(cacheKey, CACHE_MAX_AGE_MS);
  if (cached) {
    recordCacheUsage(cacheKey, cached.ageMs);
    return cached.data.map(entry => ({
