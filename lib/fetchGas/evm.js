...
async function fetchEvmGas(chain) {
  ...
  const fixed =
    chain.key === 'bsc'
      ? [buildCandidate(chain, 'fixed-3-gwei', 3)].filter(Boolean)
      : chain.key === 'polygon'
      ? [buildCandidate(chain, 'fixed-30-gwei', 30)].filter(Boolean)
      : chain.key === 'avax'
      ? [buildCandidate(chain, 'fixed-35-gwei', 35)].filter(Boolean)
      : [];

  if (fixed.length) {
    const cachedAt = setCache(cacheKey, fixed);
    return fixed.map(entry => ({ ...entry, source: 'live', cachedAt }));
  }

  recordFetchError(cacheKey, lastError || 'no evm gas candidate');
