  ...
  try {
    const result = await tryAll(
      providers.map(p => async () => {
        ...
        return parsePrice(price) ? { source: p.name, price } : null;
      }),
      timeout,
      options.totalTimeout || 4000,
    );
    if (result && parsePrice(result.price)) {
      return { priceUSD: parsePrice(result.price), updated: now.toISOString(), source: result.source, status: 'ok' };
    }
  } catch (last) {
    return {
      priceUSD: null,
      ...
      source: null,
      status: 'api-failed',
      error: last ? last.message : 'all price providers failed',
    };
  }
